...
export default function ActivityLikes({ customTimeConfig, session_id }: { customTimeConfig: any; session_id?: string; }) {
    const [likes, setLikes] = useState<any[] | null>(null);

    useEffect(() => {
        async function fetchAllLikes() {
            const fetchLikes = await getLikes({ userId: session_id || '', limit: 10 });
            ...
            {likes === null ? (
                ...
                likes.length === 0 ? (
                    <div className="flex items-center justify-center w-full border border-neutral-700 rounded-xl border-dotted h-24">
                        <p className="text-foreground">No likes</p>
                    </div>
                ) : (
                    likes.map((like: any, index: number) => (
                        <React.Fragment key={index}>
                            <Link key={like.id} className="flex items-center w-full space-x-2 p-2 group relative" href={`/@${like.user.username}/posts/${like.post.id}`}>
                                ...
                                <img src={like.user.avatarUrl || '/placeholder-image.jpg?height=40&width=40&text=user'} alt="User" className="size-10 rounded-full z-[2]"
                                    onError={(e) => {
                                            ...
                                            <div className="flex space-x-2 items-center">
                                                <p className="text-md text-zinc-400">
                                                    <span className="font-semibold text-zinc-200">
                                                        {like.user.username}
                                                    </span> liked your post</p>
                                            </div>
                                            <p className="text-sm text-zinc-400">{customTimeConfig(like.createdAt)}</p>
                                        </div>
                                        <button>
                            ...
                            {index < likes?.length - 1 && <hr className="my-4 border-border" />}
                        </React.Fragment>
                    ))
                )
            )}
